...
class QuestionBuilder {
  ...
  static createRatingQuestion(strain, key, description, scaleDescription) {
      const options = Array.from({ length: 10 }, (_, i) => `
        <input type="radio" class="btn-check" name="${key}" id="${key}${i + 1}" value="${i + 1}" autocomplete="off" aria-labelledby="${key}Label${i + 1}">
        <label class="btn btn-light" for="${key}${i + 1}" id="${key}Label${i + 1}">${i + 1}</label>
      `).join('');
      ...
      return {
        ...
        content: `
          <div class="container-fluid align-items-center text-center btn-group-container py-3">
            ...
            <div class="col-10 btn-group btn-group-sm btn-group-container" role="radiogroup" aria-labelledby="${key}_label" aria-describedby="${key}_desc">
  ...
  static createYesNoQuestion(question, key, noLabel, yesLabel) {
      return {
        title: `${question}`,
        content: `
          <div class="yes-no-question-container py-3" role="radiogroup" aria-labelledby="${key}_label">
            <div class="row g-1 text-center justify-content-center mx-0 mx-lg-3">
              <div class="col-5 mx-auto">
                <div class="form-check">
                  <input class="form-check-input" type="radio" id="${key}_no" name="${key}" value="false" aria-labelledby="${key}_no_label">
                  <label class="form-check-label" for="${key}_no" id="${key}_no_label"><strong>${noLabel}</strong></label>
                </div>
              </div>
              <div class="col-5 mx-auto">
                <div class="form-check">
                  <input class="form-check-input" type="radio" id="${key}_yes" name="${key}" value="true" aria-labelledby="${key}_yes_label">
                  <label class="form-check-label" for="${key}_yes" id="${key}_yes_label"><strong>${yesLabel}</strong></label>
                </div>
              </div>
            </div>
          </div>
        ...
        content: `
          <div class="container ps-lg-3 py-3">
            <div class="row align-items-center justify-content-center text-start g-3 mx-2">
              ...
              <div class="col-md-6 px-3">
                <select class="form-select" id="method_of_consumption" name="method_of_consumption" aria-label="Method of Consumption Dropdown Field" aria-labelledby="method_of_consumption_label">
        ...
        content: `
          <div class="container px-4 py-3">
            <div class="row px-4 py-2 py-lg-3">
              <input class="form-control text-center email-input-field" id="connoisseur" name="connoisseur" placeholder="Your email is required to rate strains" aria-labelledby="connoisseur_label" />
            </div>
          </div>`,
        key: "connoisseur"
      };
        ...
        content: `
          <div class="container align-content-center align-items-center justify-content-center text-center py-3">
            <div class="row align-items-center">
              <div class="col-7">
                <label class="pb-3 ps-2 fs-5 text-start" for="effects_explanation" id="effects_explanation_label">Please pick a minimum of two (2) to explain how ${strain} made you feel:</label>
              </div>
              <div class="col-5 align-content-center align-items-center justify-content-center">
                <select class="form-select px-3 px-lg-4" id="effects_explanation" name="effects_explanation" aria-labelledby="effects_explanation_label" multiple>
