...
@router.post("/vote/", response_model=FlowerVoteResponse)
def add_flower_vote_to_db(
    flower_vote: FlowerVoteCreate,
    db: Session = Depends(get_supa_db),
):
    return update_or_add_flower_vote(flower_vote.dict(exclude_unset=True), db=db)


@router.get("/get/", response_model=FlowersBase)
...
@router.post("/submit-hidden-flower-ranking", response_model=GetFlowerRanking)
async def submit_mystery_flower_ranking(
...
@router.post("/submit-flower-ranking", response_model=GetFlowerRanking)
...
@router.post("/submit-mystery-review", response_model=CreateMysteryFlowerReview)
async def submit_mystery_flower_review(
    mystery_flower_review: CreateMysteryFlowerReview, db: Session = Depends(get_db)
) -> MysteryFlowerReview:
    submitted_review = create_mystery_flower_review(mystery_flower_review=mystery_flower_review, db=db)
...
async def get_top_strains(db: Session = Depends(get_supa_db)):
    avg_ratings = (
        ...
        .filter(
            ...
            Flower_Ranking.cultivator == FlowerVoting.cultivator_selected,
            Flower_Ranking.strain == FlowerVoting.strain_selected,
        )
        .group_by(
            Flower_Ranking.strain,
            Flower_Ranking.cultivator,
